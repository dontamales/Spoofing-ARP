...
def spoofing_arp(ip_objetivo, widget_salida):
    ...
    try:
        ...
        # Paquete ARP de respuesta hacia la puerta de enlace
...
# Función para iniciar el ataque ARP spoofing en múltiples IPs
def iniciar_spoofing():
    global ataque_en_curso
    ips_str = entrada_ips.get("1.0", tk.END)
    # Obtener cada línea y eliminar espacios en blanco, omitiendo líneas vacías
    ...
    if lista_ips:
        ...
        for ip in lista_ips:
            hilo = threading.Thread(target=spoofing_arp, args=(ip, widget_salida), daemon=True)
            hilo.start()
...
tk.Label(ventana, text="Ingrese la lista de IP objetivo (una por línea):").pack(pady=5)
entrada_ips = tk.Text(ventana, width=30, height=10)
